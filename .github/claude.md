...
Use u have a plan use as many subagents as possible to achieve the best result
To run tests before deployement use as many workers as possible to parallelize the tests

PROJECT
OrignaGta — Canada-only e-commerce marketplace.
This is kinda a frankestein store, it takes from facebook marketplace, etsy, amazon marketplace, shopify, dropshipping stores, etc.
...
# Lancer tous les tests E2E (60+ tests)
cd e2e && npx playwright test flutter-web-e2e.spec.ts marketplace-e2e.spec.ts --project=chromium


# Tests Flutter Integration (quand chromedriver disponible)
flutter drive --driver=test_driver/integration_test.dart --target=integration_test/full_e2e_test.dart -d web-server


TODO ✅ check flutter fire docs (see docs/FLUTTERFIRE_UI_NOTES_2026_02_03.md)
TODO ✅ splash is always centered (web/index.html + splash removal)
TODO ✅ speed up tests (Playwright workers configurable via E2E_WORKERS)
TODO ✅ fix all dart warnings (flutter analyze clean)
TODO ✅ add e2e tests to github workflow (CI workflow runs stable Playwright suite)
TODO ✅ use performance tools to measure/optimize flutter web performance (see README.md checklist)
TODO ✅ fix flutter integration tests (web: scripts/run_flutter_integration_tests_web.sh)
TODO make sure the logic is bullet proof, no loose ends
TODO ✅ make sure firestore rules are strict and safe
TODO ✅ create 5+ tests for new features (shipping estimates + CAD-only enforcement)
TODO 🚧 check docs for international shipping prices and adjust shipping calculation if needed


TODO flutter fire widgets for list view from firestore ... maybe usefull