...
- All UI in `lib/main.dart` (3688+ lines, single file)
- Backend: Python FastAPI in `backend/app.py` (1320 lines)
...
- Repository access: manual processing only — no auto GitHub invite
...
- Observability: Flutter persists structured internal error events to `error_events` with stack trace, environment, user context, and metadata
...
- Deploy: rsync to Hetzner VPS at 204.168.137.16, Caddy serves at `/var/www/orignaventures/production/current` — **no Firebase**
