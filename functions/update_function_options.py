...
REPLACEMENTS = [
    ...
    (
        r'@https_fn\.on_call\(cors=cors_config\)',
        '@https_fn.on_call(**DEFAULT_OPTIONS._asdict(), cors=CORS_CONFIG)'
    ),
    # on_request without parameters -> use DEFAULT_OPTIONS
    (
        r'@https_fn\.on_request\(\)',
        '@https_fn.on_request(**DEFAULT_OPTIONS._asdict())'
    ),
]

# Import statement to add
IMPORT_LINE = "from function_options import DEFAULT_OPTIONS, WEBHOOK_OPTIONS, CORS_CONFIG, CRON_OPTIONS\n"
    ...
    if '@https_fn.' in content and 'from function_options import' not in content:
        ...
        if 'from firebase_functions import' in content:
            content = content.replace(
                ...
                IMPORT_LINE + 'from firebase_functions import',
                1
            )
