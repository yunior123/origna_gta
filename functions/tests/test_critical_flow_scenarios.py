...
class TestOrderStateMachine:
    ...
    def test_valid_transitions(self):
        ...
        for current, new in valid_cases:
            assert is_valid_order_status_transition(current, new), \
    ...
    def test_invalid_transitions(self):
        ...
        for current, new in invalid_cases:
            assert not is_valid_order_status_transition(current, new), \
...
class TestPostalCodeValidation:
    ...
    def test_valid_postal_codes(self):
        ...
        valid_codes = [
            'M5V 3A8', 'K1A 0B1', 'V6B 1A1',
            'T5J 2N4', 'H3A 1E8', 'L5B 3C2',
        ]
    ...
    def test_capture_admin_can_capture(self, mock_require, mock_db):
        ...
        try:
            ...
            assert e.code != 'permission-denied', \
                f"Admin should be able to capture! Got: {e.code}: {e.message}"
        except Exception:
...
class TestOrderStatusUpdate:
    ...
    def test_update_invalid_transition_blocked(self, mock_ts, mock_db):
        ...
        req = make_mock_request(uid='seller_001', data={
            'orderId': 'order_001',
            'newStatus': 'delivered'
        })
    ...
    def test_buyer_cannot_update_status(self, mock_ts, mock_db):
        ...
        req = make_mock_request(uid='buyer_123', data={
            'orderId': 'order_001',
            'newStatus': 'shipped'
        })
    ...
    def test_partial_refund_charge_handling(self):
        ...
        with patch('handlers.payment_stripe.get_db') as mock_db, \
            ...
            mock_db.return_value.collection.return_value\
                .where.return_value\
                .limit.return_value.stream.return_value = [order_doc]
...
class TestDisputeHandling:
    ...
    def test_dispute_uses_payment_intent_not_charge_id(self):
        ...
        with patch('handlers.payment_stripe.get_db') as mock_db, \
            ...
            # Mock orders query
            mock_orders_query = mock_db.return_value.collection.return_value.where.return_value.limit.return_value.stream
            ...
            for c in where_calls:
                if c[0][0] == 'stripePaymentIntentId':
                    assert c[0][2] == 'pi_yyy', \
                        f"Expected query with pi_yyy, got {c[0][2]}"
                    break
    ...
    def test_dispute_closed_resolution_logged(self):
        ...
        with patch('handlers.payment_stripe.get_db') as mock_db, \
            ...
            mock_db.return_value.collection.return_value.where.return_value\
                .where.return_value.limit.return_value.stream.return_value = [alert_doc]
...
class TestCronJobs:
    ...
    def test_expired_auth_uses_atomic_increment(self):
        ...
        assert 'Increment' in source, \
            "Stock restoration should use atomic Increment, not read-then-write"
...
class TestOnOrderStatusChangedEmails:
    ...
    def test_trigger_fetches_buyer_email(self):
        ...
        assert 'buyer_email' in source, \
            "Should use buyer_email variable"
        assert 'to_email=buyer_email' in source, \
    ...
    def test_trigger_handles_missing_email_gracefully(self):
        ...
        assert 'not buyer_email' in source or 'buyer_email is None' in source, \
