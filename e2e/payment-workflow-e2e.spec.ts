...
async function writeDoc(path: string, fields: any) {
  // Use updateMask to perform a MERGE (partial update) instead of replacing the whole doc
  const fieldPaths = Object.keys(fields).map(k => `updateMask.fieldPaths=${k}`).join('&');
  const res = await fetch(
    `${FIRESTORE_EMULATOR}/v1/projects/${PROJECT_ID}/databases/(default)/documents/${path}?${fieldPaths}`,
    {
      method: 'PATCH',
...
/** Build a valid checkout payload from a product doc and buyer auth */
async function buildCheckoutPayload(
  buyerUid: string, productId: string, quantity = 1
...
/** Full checkout + pay — returns { orderId, checkoutUrl, orderData } */
...
/** Wait for order to reach a status — polls Firestore */
async function waitForOrderStatus(
  orderId: string, targetStatuses: string[], field = 'orderStatus', maxWaitMs = 30_000
): Promise<any> {
  ...
  while (Date.now() - start < maxWaitMs) {
    ...
    if (doc) {
      ...
      if (targetStatuses.includes(order[field])) return order;
    }
