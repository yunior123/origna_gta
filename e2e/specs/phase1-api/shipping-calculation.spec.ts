...
function isTransientOrAuthError(e: any): boolean {
  ...
  return /rate limit|duplicate order|not available|too many|unauthenticated|internal error|failed to create payment|401|403|500/i.test(
    msg,
  );
}
...
describe("Shipping Calculation", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.error(
        "WARNING: buyer auth failed in beforeAll — all shipping tests will skip: " +
  ...
  test(
    "Checkout includes tax calculation for Ontario address",
    { timeout: 120_000 },
    async () => {
      if (!buyerAuth) {
        console.log("Skipped: buyer auth unavailable");
        return;
      }
      ...
      try {
        ...
        if (isTransientOrAuthError(e)) {
          console.log(
            "Skipped: transient/auth error — " +
      ...
      if (order.taxAmountCents != null && order.totalAmountCents != null) {
        ...
        const taxableBase =
          order.subtotalCents + (order.shippingCostCents || 0);
        const expected13pct = Math.round(taxableBase * 0.13);
        expect(
          order.taxAmountCents,
          "Ontario HST must be exactly 13%",
        ).toBeGreaterThanOrEqual(expected13pct - 1);
        expect(
          order.taxAmountCents,
          "Ontario HST must be exactly 13%",
        ).toBeLessThanOrEqual(expected13pct + 1);
      } else {
        console.log(
          "Skipped tax assertions: taxAmountCents/totalAmountCents not yet populated (awaiting Stripe webhook)",
        );
      }
    },
  );
  ...
  test(
    ...
    async () => {
      ...
      if (order.taxAmountCents != null && order.totalAmountCents != null) {
        ...
        const expectedTotal =
          order.subtotalCents + order.taxAmountCents + shippingCents;
        // Allow 1 cent rounding tolerance
        expect(
          Math.abs(order.totalAmountCents - expectedTotal),
        ).toBeLessThanOrEqual(1);
      } else {
        console.log(
          "Skipped total assertions: taxAmountCents/totalAmountCents not yet populated (awaiting Stripe webhook)",
        );
      }
    },
  );
  ...
  test("Currency is always CAD", { timeout: 120_000 }, async () => {
    ...
    try {
      ...
      if (isTransientOrAuthError(e)) {
        console.log(
          "Skipped: transient/auth error — " + (e?.message || "").slice(0, 80),
        );
        return;
      }
      throw e;
  ...
  test(
    "Multiple quantity correctly multiplies subtotal",
    { timeout: 120_000 },
    async () => {
      if (!buyerAuth) {
        console.log("Skipped: buyer auth unavailable");
      ...
      let adminAuth: any;
      try {
        adminAuth = await signIn(
          TEST_ACCOUNTS.ADMIN_EMAIL,
          TEST_ACCOUNTS.ADMIN_PASS,
        );
      } catch (e: any) {
        console.log(
          "Skipped: admin auth failed — " + (e?.message || "").slice(0, 80),
        );
        return;
      }
      ...
      try {
        const { data: data1 } = await buildCheckoutPayload(
          buyerAuth.localId,
          productId,
          1,
          buyerAuth.idToken,
        ...
        const order1 = parseDoc(
          await readDoc(`orders/${result1.orderId}`, buyerAuth.idToken),
        );

        const { data: data2 } = await buildCheckoutPayload(
          buyerAuth.localId,
          productId,
          2,
          buyerAuth.idToken,
        ...
        try {
          ...
          if (isTransientOrAuthError(e)) {
            console.log(
              "Skipped: transient/auth error on second checkout — " +
                (e?.message || "").slice(0, 80),
            );
            return;
          }
        ...
        const order2 = parseDoc(
          await readDoc(`orders/${result2.orderId}`, buyerAuth.idToken),
        );
  ...
  test(
    "Quebec address applies QST+GST tax rate (~14.975%)",
    { timeout: 120_000 },
    async () => {
      if (!buyerAuth) {
        console.log("Skipped: buyer auth unavailable");
      ...
      const order = parseDoc(
        await readDoc(`orders/${result.orderId}`, buyerAuth.idToken),
      );
      ...
      if (order.taxAmountCents != null) {
        ...
        expect(
          order.taxAmountCents,
          "QC tax must be ~14.975%",
        ).toBeGreaterThanOrEqual(expectedQC - 6);
        expect(
          order.taxAmountCents,
          "QC tax must be ~14.975%",
        ).toBeLessThanOrEqual(expectedQC + 6);
      } else {
        console.log(
          "Skipped QC tax assertions: taxAmountCents not yet populated (awaiting Stripe webhook)",
        );
      }
    },
  );
  ...
  test(
    "Alberta address applies GST-only tax rate (5%)",
    { timeout: 120_000 },
    async () => {
      if (!buyerAuth) {
        console.log("Skipped: buyer auth unavailable");
      ...
      if (order.taxAmountCents != null) {
        ...
        expect(
          order.taxAmountCents,
          "AB tax must be exactly 5% GST",
        ).toBeGreaterThanOrEqual(expected5pct - 1);
        expect(
          order.taxAmountCents,
          "AB tax must be exactly 5% GST",
        ).toBeLessThanOrEqual(expected5pct + 1);
      } else {
        console.log(
          "Skipped AB tax assertions: taxAmountCents not yet populated (awaiting Stripe webhook)",
        );
      }
    },
  );
  ...
  test(
    "Perishable item from local seller: checkout succeeds with same-day option",
    { timeout: 120_000 },
    async () => {
      let adminAuth: any;
      try {
      ...
      await writeDoc(
        `products/${productId}`,
        {
          ...
          deliveryOptions: [
            { type: "same_day", national: false, estimatedDays: 0 },
          ],
          estimatedShipDays: 0,
          dateCreated: new Date().toISOString(),
        },
  ...
  test(
    "Local-only item: checkout blocked for out-of-province buyer",
    { timeout: 120_000 },
    async () => {
      let adminAuth: any;
      try {
      ...
      try {
        ...
        if (result.error) {
          const rawCode = result.error?.code ?? result.error;
          const code = typeof rawCode === "string" ? rawCode.toLowerCase() : rawCode;
          expect([
            "failed-precondition",
            "invalid-argument",
            "validation-error",
  ...
  test(
    "Perishable product without local/same-day option is auto-deactivated by backend",
    { timeout: 120_000 },
    async () => {
      ...
      await writeDoc(
        `products/${productId}`,
        {
          ...
          description:
            "Perishable item with only standard shipping — should be deactivated.",
          price: 5.0,
          priceCents: 500,
      ...
      try {
        ...
        const product = parseDoc(
          await readDoc(`products/${productId}`, adminAuth.idToken),
        );
        ...
        expect(
          isActive,
          "Perishable product without local/same-day must be deactivated",
        ).toBe(false);
      } finally {
        await deleteDoc(`products/${productId}`, adminAuth.idToken).catch(
          () => {},
  ...
  test(
    "International seller has non-zero shipping cost",
    { timeout: 120_000 },
    async () => {
      if (!buyerAuth) {
        console.log("Skipped: buyer auth unavailable");
      ...
      try {
        ...
        console.log(
          "Skipped: cannot check subscription status — " +
            (e?.message || "").slice(0, 80),
        );
        return;
      }
      ...
      if (subData.isPremium) {
        console.log(
          "Skipping: buyer is premium — shipping is always free for premium accounts",
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          "Skipped: could not discover products — " +
            (e?.message || "").slice(0, 80),
        );
        return;
      }
      const intlProduct = products.find(
        (p) => p.id === "e2e_product_intl_seller",
      );
      if (!intlProduct) {
        console.log("Skipped: international test product missing");
        return;
      }
      ...
      if (order.shippingCostCents == null) {
        console.log(
          "Skipped: shippingCostCents undefined (not yet populated by backend)",
        );
        return;
      }
