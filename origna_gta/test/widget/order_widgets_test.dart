...
@GenerateNiceMocks([MockSpec<OrderRepository>(), MockSpec<CartController>()])
...
void main() {
  ...
  Widget buildTestWidget(Widget child, WidgetTester tester) {
    ...
    return TestWrapper(
      ...
      onGenerateRoute: (settings) => MaterialPageRoute(
        builder: (_) => const Scaffold(body: Text('Dummy Route')),
      ),
  ...
  group('OrderWidgets Component Smoke Tests', () {
    ...
    testWidgets('renders DigitalItemActions with license key', (tester) async {
      ...
      await tester.pumpWidget(
        buildTestWidget(DigitalItemActions(item: digitalItem), tester),
      );
      await tester.pump();
    ...
    testWidgets('renders OrderStatusTimeline', (tester) async {
      await tester.pumpWidget(
        buildTestWidget(const OrderStatusTimeline(currentStep: 2), tester),
      );
      await tester.pump();
    ...
    testWidgets('renders PendingApprovalsBanner', (tester) async {
      await tester.pumpWidget(
        buildTestWidget(const PendingApprovalsBanner(count: 5), tester),
      );
      await tester.pump();
    ...
    testWidgets('renders SellerPackageTimeline', (tester) async {
      await tester.pumpWidget(
        buildTestWidget(const SellerPackageTimeline(currentStep: 1), tester),
      );
      await tester.pump();
    ...
    testWidgets('renders SoftwareDownloadLinks', (tester) async {
      ...
      await tester.pumpWidget(
        buildTestWidget(SoftwareDownloadLinks(item: softwareItem), tester),
      );
      await tester.pump();
  ...
  group('BuyerOrderCard Integration', () {
    ...
    testWidgets('renders detailed order view', (tester) async {
      await tester.pumpWidget(
        buildTestWidget(
          BuyerOrderCard(order: mockOrder, isDetailView: true),
          tester,
        ),
      );
    ...
    testWidgets('reorder items interaction', (tester) async {
      final deliveredOrder = mockOrder.copyWith(
        orderStatus: gen.OrderStatus.delivered,
      );
      when(
      ...
      await tester.pumpWidget(
        buildTestWidget(BuyerOrderCard(order: deliveredOrder), tester),
      );
      await tester.pump(const Duration(seconds: 1));
