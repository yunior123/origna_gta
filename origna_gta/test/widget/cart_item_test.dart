...
void main() {
    ...
    return TestWrapper(
      overrides: [
        ...
        cartItemQuantityProvider(
          'cart_123',
        ).overrideWith((ref) => AsyncValue.data(quantity)),
      ],
      child: Scaffold(
        body: CartItemScreen(
  ...
  group('CartItemScreen Widget Tests', () {
    ...
    testWidgets('shows digital delivery label for digital items', (
      tester,
    ) async {
      final digitalItem = Map<String, dynamic>.from(testItem)
    ...
    testWidgets('calculates total price correctly for multiple quantity', (
      tester,
    ) async {
    ...
    testWidgets('can save for later', (tester) async {
      ...
      await tester.tap(find.byTooltip('btn-save-for-later'));
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 500));
    ...
    testWidgets('can remove item via button', (tester) async {
      ...
      await tester.pumpWidget(
        createTestWidget(item: testItem, onRemove: () => removed = true),
      );
      await tester.pump();
      await tester.pump(const Duration(seconds: 1));

      await tester.tap(find.byTooltip('btn-remove-cart-item'));
    ...
    testWidgets('can remove item via swipe', (tester) async {
      ...
      final noImageItem = Map<String, dynamic>.from(testItem)
        ..[Fields.imageUrls] = [];
      await tester.pumpWidget(
        createTestWidget(item: noImageItem, onRemove: () => removed = true),
      );
      await tester.pump();
      await tester.pump(const Duration(seconds: 1));
    ...
    group('CartItemScreen Image Rendering', () {
      ...
      testWidgets('renders single image', (tester) async {
        final singleImageItem = Map<String, dynamic>.from(testItem)
          ..[Fields.imageUrls] = ['https://example.com/img1.jpg'];
      ...
      testWidgets('renders multiple images with counter', (tester) async {
        final multiImageItem = Map<String, dynamic>.from(testItem)
