...
void main() {
  ...
  group('Data Transformation Utilities', () {
    test('dynamicToDateTime', () {
      final dt = DateTime(2020, 1, 1);
      expect(dynamicToDateTime(dt), dt);

      final fromString = dynamicToDateTime('2020-01-01');
      expect(fromString, DateTime(2020, 1, 1));

      final fallback = dynamicToDateTime('invalid');
      expect(fallback, isA<DateTime>());
    });
  ...
  group('AppError', () {
    ...
    test('getMessage returns backend message for OrignaBaseException', () {
      final error = OrignaBaseException('Custom error', statusCode: 500);
    ...
    test('getMessage for OrignaBaseAuthException returns safe message', () {
      final error = OrignaBaseAuthException(code: 'wrong-password');
      final msg = AppError.getMessage(error);
  ...
  group('dynamicToTimestamp - edge cases', () {
    test('null returns DateTime.now()', () {
    ...
    test('empty string returns DateTime.now() fallback', () {
  ...
  group('AppError.getMessage - extended', () {
    ...
    test('getMessage for OrignaBaseException with query index leak', () {
      final error = OrignaBaseException(
        'The query requires an index you can find at...',
        statusCode: 500,
      );
      final msg = AppError.getMessage(error);
