...
void main() {
  ...
  group('SellerOrdersViewModel Unit Tests', () {
    ...
    test(
      'updateShippingAndCapture calls repository and updates state',
      () async {
        when(
          mockRepo.updateShippingCost(any, any, any),
        ).thenAnswer((_) async => Future.value());
        when(
          mockRepo.updateItemStatus(
            any,
            any,
            any,
            trackingNumber: anyNamed('trackingNumber'),
            carrier: anyNamed('carrier'),
            carrierNote: anyNamed('carrierNote'),
          ),
        ).thenAnswer((_) async => Future.value());

        final viewModel = container.read(
          sellerOrdersViewModelProvider.notifier,
        );
        await viewModel.updateShippingAndCapture(
          'order_123',
          1500,
          'TRK123',
          carrier: 'FedEx',
    ...
    test('updateItemStatus calls repository and updates state', () async {
      ...
      await viewModel.updateItemStatus(
        'order_123',
        'item_456',
