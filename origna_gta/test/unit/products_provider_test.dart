...
void main() {
  ...
  group('Favorites Providers', () {
    test('favoritesProvider returns stream from repository', () async {
      when(
        mockRepo.watchFavorites('test_user'),
      ).thenAnswer((_) => Stream.value({'p1', 'p2'}));
    ...
    test('favoritedProductsProvider fetches in chunks', () async {
      ...
      when(
        mockRepo.watchFavorites('test_user'),
      ).thenAnswer((_) => Stream.value(ids));

      final chunk1 = List.generate(30, (i) => 'id_$i');
      final chunk2 = List.generate(5, (i) => 'id_${i + 30}');

      when(mockRepo.fetchProductsByIds(chunk1)).thenAnswer(
      ...
      when(mockRepo.fetchProductsByIds(chunk2)).thenAnswer(
        (_) async => List.generate(
          5,
          (i) => Product(
            productId: 'id_${i + 30}',
  ...
  group('Family Providers', () {
    ...
    test('similarProductsProvider fetches correctly', () async {
      ...
      final result = await container.read(
        similarProductsProvider((excludeProductId: 'p1', categoryId: 1)).future,
      );
