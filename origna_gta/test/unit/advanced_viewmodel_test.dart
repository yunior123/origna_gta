...
void main() {
  group('Tests de Logique Panier', () {
    test('Ajout du même produit devrait incrémenter la quantité', () {
      ...
      final existingIndex = items.indexWhere(
        (item) => item['productId'] == 'prod_1',
      );
      if (existingIndex != -1) {
        items[existingIndex]['quantity'] =
            (items[existingIndex]['quantity'] as int) + 3;
      }
    ...
    test('Le total du panier calcule correctement avec plusieurs articles', () {
      ...
      for (final item in items) {
        totalCents += (item['priceCents'] as int) * (item['quantity'] as int);
      }

      // Total: 5000 + 10650 = 15650 cents = $156.50
      expect(totalCents / 100.0, closeTo(156.50, 0.01));
    });
  ...
  group('Tests de Logique Commande', () {
    ...
    test('SÉCURITÉ: La commande valide la manipulation de prix', () {
      ...
      bool validateOrder(
        Map<String, dynamic> clientOrder,
        Map<String, double> serverPrices,
      ) {
      ...
      expect(
        validateOrder(clientOrder, serverPrices),
        false,
    ...
    test('La commande divise un panier multi-vendeur correctement', () {
      ...
      // Calculer total pour seller_A in cents
      int sellerATotalCents = 0;
      for (final item in ordersBySeller['seller_A']!) {
        sellerATotalCents +=
            (item['priceCents'] as int) * (item['quantity'] as int);
      }
      // (2x5000) + (1x2000) = 12000 cents = $120.00
      expect(sellerATotalCents / 100.0, closeTo(120.00, 0.01));
    });
  });
    ...
    test('Validation des champs requis d\'un produit', () {
      ...
      bool validateProduct(Map<String, dynamic> product) {
        return product.containsKey('id') &&
            ...
            product.containsKey('priceCents') &&
            product.containsKey('sellerId') &&
            (product['priceCents'] as int) > 0 &&
      ...
      final invalidProduct = {
        'id': 'prod_456',
        'name': '',
        'priceCents': -1000, // Prix négatif invalide
      };
  ...
  group('Tests de Recherche de Produits', () {
    test('La recherche filtre par catégorie correctement', () {
      ...
      expect(
        electronicsProducts.every((p) => p['category'] == 'Électronique'),
        true,
      );
    });

    test(
      'CAS LIMITE: Requête de recherche vide retourne tous les produits',
      () {
        final allProducts = [
  ...
  group('Tests d\'Utilitaires et Validation', () {
    ...
    test('Conversion centimes en euros', () {
      ...
      expect(
        centsToEuros(eurosToCents(originalEuros)),
        closeTo(originalEuros, 0.01),
      );
    });
    ...
    test('Troncature de description longue', () {
      ...
      final longText =
          'Ceci est une très longue description de produit qui doit être tronquée';
  ...
  group('Tests de Notifications et Alertes', () {
    test('Alerte de stock faible se déclenche correctement', () {
      ...
      expect(
        shouldTriggerLowStockAlert(0, 10),
        false,
      ); // Rupture de stock (différent)
