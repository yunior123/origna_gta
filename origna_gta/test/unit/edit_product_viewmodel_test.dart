  ...
  group('EditProductViewModel', () {
    test('initializes state correctly from product', () {
      final state = container.read(
        editProductViewModelProvider(initialProduct),
      );
    ...
    test('onStreetChanged fetches suggestions', () async {
      ...
      when(
        mockRepo.getAutocompleteSuggestions(any),
    ...
    test('setExistingImageAsCover moves image to front', () {
      final product = initialProduct.copyWith(
        imageUrls: ['img1', 'img2', 'img3'],
      );
      final viewModel = container.read(
        editProductViewModelProvider(product).notifier,
      );
    ...
    test('unauthorized user cannot update product', () async {
      ...
      container = ProviderContainer(
        overrides: [
          productRepositoryProvider.overrideWithValue(mockRepo),
          userIdProvider.overrideWith((ref) => 'user_999'),
        ],
      );
    ...
    test(
      'successful update calls repository with custom delivery options',
      () async {
        final viewModel = container.read(
          editProductViewModelProvider(initialProduct).notifier,
        );
        ...
        final verification = verify(
          mockRepo.updateProduct(initialProduct.productId, captureAny),
        );
    ...
    test('validation: price limits', () async {
      ...
      expect(
        container
            .read(editProductViewModelProvider(initialProduct))
            .errorMessage,
        contains('product.please_enter_price'),
      );
    });
    ...
    test('validation: compareAtPrice logic', () async {
      ...
      expect(
        container
            .read(editProductViewModelProvider(initialProduct))
            .errorMessage,
        contains('product.compare_at_price_must_be_higher'),
      );
    });
    ...
    test('validation: unverified address (null coords)', () async {
      ...
      final productNoCoords = initialProduct.copyWith(
        sellerAddress: initialProduct.sellerAddress?.copyWith(
          latitude: null,
          longitude: null,
        ),
      );
      final vmNoCoords = container.read(
        editProductViewModelProvider(productNoCoords).notifier,
      );
      ...
      expect(
        container
            .read(editProductViewModelProvider(productNoCoords))
            .errorMessage,
    ...
    test('validation: book URL invalid', () async {
      ...
      expect(
        container
            .read(editProductViewModelProvider(initialProduct))
            .errorMessage,
        contains('product.book_url_https_required'),
      );
    ...
    test('validation: no images left', () async {
      ...
      expect(
        container
            .read(editProductViewModelProvider(initialProduct))
            .errorMessage,
        contains('least one product image'),
      );
    });
    ...
    test('successful update with new images and video', () async {
      ...
      when(
        mockRepo.uploadImages(any, any),
      ).thenAnswer((_) async => ['new_image_url.jpg']);
      when(
    ...
    test('updateProduct failure on video too long', () async {
      ...
      expect(
        container
            .read(editProductViewModelProvider(initialProduct))
            .errorMessage,
        contains('product.video_too_long'),
      );
    });

    test(
      'updateProduct failure on delivery option missing when required',
      () async {
        final viewModel = container.read(
          editProductViewModelProvider(initialProduct).notifier,
        );
        ...
        expect(
          container
              .read(editProductViewModelProvider(initialProduct))
              .errorMessage,
          contains('at least one delivery option'),
        );
      },
    );
    ...
    test('updateProduct failure on compareAtPrice < price', () async {
      ...
      await viewModel.updateProduct(
        ...
        shipDays: 3,
        deliveryOptions: [],
        compareAtPriceCents: 5000, // Must be higher
      );
      expect(
        container
            .read(editProductViewModelProvider(initialProduct))
    ...
    test('digital software validation failure cases', () async {
      ...
      expect(
        container
            .read(editProductViewModelProvider(initialProduct))
            .errorMessage,
        contains('Add at least one platform download URL'),
      );
