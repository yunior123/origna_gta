  ...
  group('OrderRepository Tests', () {
    test('fetchOrderById returns order', () async {
      final fakeOrder = gen.Order(
        ...
        shippingAddress: const gen.Address(street: 'S', city: 'C', state: 'ON', postalCode: 'M1M 1M1', country: 'CA'),
        currency: 'cad',
        sellerIds: const ['s1'],
        stripeSessionId: 'sess_1',
      );

      when(mockRepository.fetchOrderById('o1')).thenAnswer((_) async => fakeOrder);
    ...
    test('watchBuyerOrders returns stream', () {
      when(mockRepository.watchBuyerOrders('u1')).thenAnswer((_) => Stream.value([]));
