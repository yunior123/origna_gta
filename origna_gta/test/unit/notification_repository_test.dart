...
void main() {
  ...
  setUp(() {
    ...
    when(mockOrignaBase.collection(Collections.users)).thenReturn(mockCollection);
    when(mockCollection.doc(userId)).thenReturn(mockUserDoc);
    when(mockUserDoc.subcollection(Collections.notifications)).thenReturn(mockNotifications);
    when(mockNotifications.doc('n1')).thenReturn(mockNotificationDoc);
    when(mockNotifications.where(Fields.isRead, isEqualTo: false)).thenReturn(mockQuery);
    when(mockNotifications.orderBy(Fields.createdAt, descending: true)).thenReturn(mockQuery);
  ...
  group('NotificationRepository', () {
    test('markRead updates a single notification', () async {
      when(mockNotificationDoc.update({Fields.isRead: true}))
    ...
    test('markAllRead updates all unread notifications', () async {
      ...
      when(mockBatch.commit()).thenAnswer((_) async => <Map<String, dynamic>>[]);
    ...
    test('markAllRead skips batch commit when there are no unread notifications', () async {
    ...
    test('watchNotifications emits ordered notifications with stable ids', () async {
      ...
      when(mockNotifications.snapshots()).thenAnswer((_) => controller.stream);

      Future<void>.microtask(() {
        controller.add(DocumentChange(type: ChangeType.update, document: firstDoc));
      });
      ...
      verify(mockNotifications.orderBy(Fields.createdAt, descending: true)).called(2);
