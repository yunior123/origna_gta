...
void main() {
  group('ErrorEventService.buildPayload', () {
    test(
      'builds a support-grade payload with identifiers, auth context, and metadata',
      () {
        ...
        expect(metadata['reproSteps'], <String>[
          'open login',
          'submit bad password',
        ]);
    ...
    test('truncates oversized message and stack trace fields', () {
      ...
      expect(
        (payload[Fields.errorMessage] as String).length,
        lessThanOrEqualTo(4003),
      );
      expect(
        (payload[Fields.stackTrace] as String).length,
        lessThanOrEqualTo(12003),
      );
