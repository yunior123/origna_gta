...
class _FakeProductRepo implements ProductRepository {
  ...
  Future<void> submitRatingAtomic(String orderId, String productId, int rating,
      {List<Uint8List>? reviewImages, String? reviewText}) async {
    submitCalls++;
...
void main() {
  ...
  group('ProductRatingViewModel', () {
    ...
    test('setReviewText updates state', () {
      container.read(productRatingViewModelProvider.notifier).setReviewText('Good!');
      expect(container.read(productRatingViewModelProvider).reviewText, 'Good!');
    });

    test('submitRating returns false for rating < 1', () async {
      final result = await container.read(productRatingViewModelProvider.notifier).submitRating('o1', 'p1', 0);
      expect(result, isFalse);
      expect(container.read(productRatingViewModelProvider).errorMessage, isNotNull);
      expect(fakeRepo.submitCalls, 0);
    });

    test('submitRating returns false for rating > 5', () async {
      final result = await container.read(productRatingViewModelProvider.notifier).submitRating('o1', 'p1', 6);
      expect(result, isFalse);
      expect(fakeRepo.submitCalls, 0);
    });

    test('submitRating returns true on success', () async {
      final result = await container.read(productRatingViewModelProvider.notifier)
    ...
    test('submitRating sets errorMessage on failure', () async {
      ...
      final result = await container.read(productRatingViewModelProvider.notifier).submitRating('o1', 'p1', 3);
      expect(result, isFalse);
      expect(container.read(productRatingViewModelProvider).errorMessage, isNotNull);
    });
    ...
    test('submitRating passes images to repository', () async {
      ...
      await container.read(productRatingViewModelProvider.notifier)
