    ...
    () {
        ...
        () async {
          ...
          try {
            ...
            expect(
              ...
              isTrue,
              reason: 'Review submission error should be expected, got ${e.statusCode}: ${e.message}',
            );
          }
        },
        ...
        () async {
          ...
          try {
            ...
            expect(
              [400, 403, 404, 409, 422].contains(e.statusCode),
              isTrue,
              reason: 'Return request error should be expected, got ${e.statusCode}: ${e.message}',
            );
          }
        },
        timeout: const Timeout(Duration(minutes: 2)),
            ...
            final result = await ob.request(
              ...
              body: {
                ...
                'updates': productIds
                    .map((id) => {
                          Fields.productId: id,
                          Fields.description: 'Bulk update test — ${DateTime.now().toIso8601String()}',
                        })
                    .toList(),
              },
            );
      ...
      test(
        ...
        () async {
          ...
          try {
            await ob.request('POST', ApiEndpoints.productsBulkUpdate, body: {
              Fields.userId: ob.auth.currentUserId,
            ...
            expect(
              ...
              isTrue,
              reason: 'Too many items should be rejected, got ${e.statusCode}: ${e.message}',
            );
          }
        },
        timeout: const Timeout(Duration(minutes: 2)),
        ...
        () async {
          ...
          try {
            ...
            expect(
              ...
              isTrue,
              reason: 'Export data error should be expected, got ${e.statusCode}: ${e.message}',
            );
          }
        },
        timeout: const Timeout(Duration(minutes: 2)),
      ...
      test(
        ...
        () async {
          ...
          try {
            ...
            final hasSecret = result.containsKey(ApiKeys.secret) ||
                result.containsKey(ApiKeys.qrCodeUrl) ||
                result.containsKey(ApiKeys.provisioningUri);
            expect(hasSecret, isTrue,
                reason: 'MFA enroll should return secret/QR. Keys: ${result.keys.toList()}');
          } on OrignaBaseException catch (e) {
            // 409 = already enrolled, 400 = bad request
            expect(
              [400, 403, 404, 409, 422].contains(e.statusCode),
              isTrue,
              reason: 'MFA enroll error should be expected, got ${e.statusCode}: ${e.message}',
            );
          }
        },
        timeout: const Timeout(Duration(minutes: 2)),
      ...
      test(
        ...
        () async {
          ...
          if (hits2.isNotEmpty) {
            ...
            expect(ids1.intersection(ids2).length, lessThan(3),
      ...
      test(
        ...
        () async {
          ...
          // Read
          var doc =
              await ob.collection(Collections.addresses).doc(addressId).get();
          ...
          await ob
              ...
              .doc(addressId)
              .update({'city': 'Gatineau', 'province': 'QC'});
