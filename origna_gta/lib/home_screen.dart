...
class _HomeScreenState extends State<HomeScreen> {
            ...
            SliverPadding(
              ...
              sliver: SliverGrid(
                gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  crossAxisCount: 2, // Changed to 2 for better mobile visibility
                  crossAxisSpacing: 12, 
                  mainAxisSpacing: 12,
        ...
        child: SafeArea(
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                const Text('OrignaGta', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 22)),
                Row(
                  children: [
                    if (widget.userModel?.roles.contains('seller') ?? false)
                      _buildIconButton(icon: Icons.add_box_outlined, onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AddProductScreen()))),
  ...
  Widget _buildCartBadge(int count) {
    return Stack(
      children: [
        _buildIconButton(icon: Icons.shopping_cart_outlined, onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CartScreen()))),
        if (count > 0)
          Positioned(
            right: 0,
            top: 0,
            child: Container(
              ...
              child: Text('$count', style: const TextStyle(color: Color(0xFFFF6B35), fontSize: 10, fontWeight: FontWeight.bold)),
            ),
          ),
      ],
    );
