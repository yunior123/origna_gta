...
class ShippingApprovalScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Shipping Approvals')),
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            ...
            .where('shippingApprovalStatus', isEqualTo: ShippingApprovalStatus.pending.value)
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return _ApprovalCard(orderId: doc.id, data: orderData);
            },
          );
        },
      ),
...
class _ApprovalCardState extends State<_ApprovalCard> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result.data['message'] ?? (approved ? 'Shipping approved' : 'Order cancelled')),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(e.message ?? 'Failed to process approval'),
  ...
  Widget build(BuildContext context) {
    ...
    final estimatedShipping = (widget.data['estimatedShipping'] ?? 0.0).toDouble();
    final actualShipping = (widget.data['actualShipping'] ?? 0.0).toDouble();
    final pendingTotal = (widget.data['pendingTotal'] ?? 0.0).toDouble();
    final originalTotal = (widget.data['total'] ?? 0.0).toDouble();
