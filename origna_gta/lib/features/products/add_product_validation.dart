...
String? validateAddProductInputs({
  ...
  if (description.trim().isEmpty) return 'product.please_enter_description'.tr();
  if (description.trim().length < 10) return 'product.description_too_short'.tr();
  if (description.trim().length > 4000) return 'product.description_too_long'.tr();
  ...
  if (!state.isDigital && state.selectedWarehouseIds.isEmpty) {
    ...
    if (!postalRegex.hasMatch(normalizedPostal)) return 'product.invalid_postal'.tr();
  ...
  if (state.imageModels.isEmpty && !isDevOrTestRun) return 'product.image_required'.tr();
  ...
  if (state.hasVariants) {
    ...
    final invalidVariants = state.variants.where((v) => v.priceCents == null || v.priceCents! < 99);
    if (invalidVariants.isNotEmpty) return 'product.variant_price_required'.tr();
  }
...
String? _validateDigitalProduct(AddProductState state) {
  ...
  if (state.digitalType == DigitalTypeValues.software) {
    ...
    if (!url.startsWith('https://')) return 'product.book_url_https_required'.tr();
  }
