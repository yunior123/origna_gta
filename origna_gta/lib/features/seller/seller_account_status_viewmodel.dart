...
final sellerAccountStatusProvider =
    StreamProvider.autoDispose<SellerAccountStatus>((ref) {
      final uid = ref.watch(obUserIdProvider);
      if (uid == null) {
        return Stream.error(Exception('seller.please_login'.tr()));
      }
...
final refreshSellerStatusProvider = FutureProvider.family
    .autoDispose<SellerAccountStatus, void>((ref, _) async {
      final uid = ref.read(obUserIdProvider);
      if (uid == null) {
        throw Exception('seller.please_login'.tr());
      }
      ...
      try {
        ...
        final requirementsDue = data[ApiKeys.requirementsCurrentlyDue] is List
            ? (data[ApiKeys.requirementsCurrentlyDue] as List<dynamic>)
                  .map((e) => e.toString())
                  .toList()
            : <String>[];

        AppLogger.d(
          'Stripe Status - charges: $chargesEnabled, payouts: $payoutsEnabled',
          tag: 'seller',
        );
