...
class ProductModel {
  ...
  factory ProductModel.fromMap(Map<String, dynamic> map) {
    return ProductModel(
      ...
      //stockQuantity: map.containsKey('stockQuantity') ? int.parse(map['stockQuantity']) : 0,
...
class CartItemDetailModel {
  ...
  CartItemDetailModel({
    ...
    required this.sellerAddress, required this.sellerId,
  });
  Map<String, dynamic> toMap() {
    return {
      'productId': productId,
...
class OrderModel {
  ...
  factory OrderModel.fromDocument(DocumentSnapshot doc) {
    ...
    return OrderModel(
      ...
      currency: data["currency"], sellerIds: data["sellerIds"],
    );
  }
...
Future<void> addToCart({required String productId, required int quantity, required BuildContext context}) async {
  ...
  try {
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      final cartData = snapshot.data()?['cart'] ?? [];
      ...
      if (index != -1) {
        ...
        cartItems.add(CartModel(productId: productId, quantity: quantity, dateCreated: DateTime.now()).toMap());
      }

      transaction.update(userDocRef, {'cart': cartItems});
    });
