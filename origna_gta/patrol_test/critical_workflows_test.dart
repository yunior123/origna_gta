...
void main() {
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF01: Complete purchase — browse → cart → checkout → order success → view orders',
    ($) async {
      await createApp($);
      ...
      final productId = await _seedProduct(
        sellerId: kSeller1Uid,
        name: 'Patrol Test Scarf WF01',
        price: 45.99,
      );
      ...
      final found = await waitForText($, 'Proceed to Checkout',
      ...
      final hasCheckoutContent =
          $('Delivery Address').exists ||
          $('Add Address').exists ||
          $('Digital delivery').exists ||
          $('Place Order').exists;
      expect(hasCheckoutContent, isTrue,
          reason: 'Checkout screen should display delivery info or place order');
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF02: Seller creates and publishes a product',
    ($) async {
      await createApp($);
      ...
      final hasForm = nameField.exists ||
          $(const Key('product_price_field')).exists;
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF03: Order lifecycle — confirmed → shipped → delivered status tracking',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF04: New user registration — signup form → accept terms → account created',
    ($) async {
      await createApp($);
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF05: Product search — type query → view results → open product details',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
      ...
      await _seedProduct(
        sellerId: kSeller1Uid,
        name: 'Patrol Organic Maple Syrup',
        price: 22.50,
      );
      await $.pump(const Duration(seconds: 1));
      ...
      if (cards.exists) {
        ...
        final hasProductDetail =
            $('Add to Cart').exists ||
            $('This is your product').exists ||
            $(Icons.shopping_cart_checkout).exists;
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF06: Address management — navigate → add/edit address → verify saved',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
      ...
      final hasAddressAction =
          addAddrSemantic.evaluate().isNotEmpty ||
          editAddrSemantic.evaluate().isNotEmpty;
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF07: Favorites — add product → view favorites list → verify presence',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
      ...
      debugPrint(
          'WF07: Favorites empty state shown: ${emptyFav.exists}');
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF08: Seller manages orders — view pending → confirm shipping',
    ($) async {
      await createApp($);
      await ensureLoggedInAsSeller($);
      await $.pump(const Duration(seconds: 2));
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF09: Cart management — update quantity and remove items',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
      ...
      final prod1 = await _seedProduct(
        sellerId: kSeller1Uid,
        name: 'WF09 Cart Item A',
        price: 19.99,
      );
      final prod2 = await _seedProduct(
        sellerId: kSeller2Uid,
        name: 'WF09 Cart Item B',
        price: 34.99,
      );
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF10: Digital product purchase — no shipping required',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
      ...
      final checkout = await waitForText($, 'Proceed to Checkout',
          timeout: const Duration(seconds: 8));
      if (checkout) {
        await $('Proceed to Checkout').first.tap();
        await $.pump(const Duration(seconds: 3));
      ...
      final hasDigitalIndicator =
          $('Digital delivery').exists ||
          $('Place Order').exists;

      expect($(Scaffold), findsWidgets);
      debugPrint(
          'WF10: Digital delivery indicator: $hasDigitalIndicator');
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF11: Seller onboarding — profile → become seller → registration form',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF12: Delivery confirmed — order delivered → buyer sees delivery status',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF13: Profile navigation — verify all menu items accessible',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
      ...
      expect(foundCount, greaterThanOrEqualTo(3),
          reason: 'Should find at least 3 profile menu items');
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF14: Multi-seller cart — products from 2 sellers → checkout breakdown',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
      ...
      final prod1 = await _seedProduct(
        sellerId: kSeller1Uid,
        name: 'WF14 Seller1 Scarf',
        price: 45.99,
      );
      final prod2 = await _seedProduct(
        sellerId: kSeller2Uid,
        name: 'WF14 Seller2 Incense',
        price: 24.99,
      );
  ...
  // ──────────────────────────────────────────────────────────────────
  patrol(
    'WF15: Legal — view terms of service and privacy policy',
    ($) async {
      await createApp($);
      await ensureLoggedInAsBuyer($);
      await $.pump(const Duration(seconds: 2));
