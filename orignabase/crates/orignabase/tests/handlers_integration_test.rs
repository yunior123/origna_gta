...
async fn test_241_auth_register_duplicate_email() {
    ...
    let (status1, _) = make_request(
        ...
        "/auth/register",
        None,
        Some(json!({ "email": email, "password": "REDACTED_TEST_PASSWORD" })),
    )
    .await;
...
async fn test_243_auth_register_invalid_email() {
    ...
    let (status, _) = make_request(
        ...
        "/auth/register",
        None,
        Some(json!({ "email": "not-an-email", "password": "REDACTED_TEST_PASSWORD" })),
    )
    .await;
...
async fn test_245_auth_login_nonexistent_user() {
    ...
    let (status, _) = make_request(
        ...
        "/auth/login",
        None,
        Some(json!({ "email": "nobody@nonexistent.com", "password": "REDACTED_TEST_PASSWORD" })),
    )
    .await;
