...
mod tests {
    ...
    async fn test_ensure_customer_returns_existing_customer_id() {
        ...
        let result = ensure_customer(&state, "user_1", "STRIPE_SECRET_KEY_REDACTED")
            .await
            .unwrap();
    ...
    async fn test_ensure_customer_stripe_create_failure() {
        ...
        let err = ensure_customer(&state, "user_1", "STRIPE_SECRET_KEY_REDACTED")
            .await
            .unwrap_err();
    ...
    async fn test_create_subscription_with_payment_method_full_flow() {
        ...
        let Json(resp) = create_subscription(
            State(state.clone()),
            Extension(auth("users:user_pm")),
            Json(CreateSubscriptionRequest {
                user_id: Some("users:user_pm".to_string()),
