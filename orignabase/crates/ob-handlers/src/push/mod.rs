...
async fn get_access_token(
    ...
    if !resp.status().is_success() {
        return Err(PushError::OAuth2("FCM token refresh failed".into()));
    }
...
pub async fn send_push(
    ...
    if REDACTED_SECRET_json.is_empty() {
        return Err(PushError::MissingServiceAccount);
    }

    let access_token = get_access_token(http_client, REDACTED_SECRET_json).await?;
