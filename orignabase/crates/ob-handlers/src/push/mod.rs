...
async fn get_access_token(
    ...
    REDACTED_SECRET_json: &str,
) -> Result<String> {
    let sa: ServiceAccountKey = serde_json::from_str(REDACTED_SECRET_json)
...
/// - `REDACTED_SECRET_json` — JSON string of the service account key
...
async fn send_push_internal(request: PushRequest<'_>) -> Result<()> {
    if request.REDACTED_SECRET_json.is_empty() {
...
mod tests {
    ...
    fn REDACTED_SECRET_json(token_uri: &str) -> String {
        serde_json::json!({
    ...
    #[tokio::test]
    async fn test_send_push_missing_REDACTED_SECRET() {
    ...
    #[tokio::test]
    async fn test_send_push_invalid_REDACTED_SECRET() {
        let client = reqwest::Client::new();
    ...
    #[test]
    fn test_REDACTED_SECRET_key_parsing() {
        let json = r#"{
            "client_email": "test@project.iam.gserviceaccount.com",
    ...
    async fn test_send_push_oauth_network_error() {
        let client = reqwest::Client::new();
        let sa_json = REDACTED_SECRET_json("http://127.0.0.1:1");
        let result = send_push(
            &client,
            "my-project",
            &sa_json,
    ...
    #[test]
    fn test_REDACTED_SECRET_key_missing_fields_fails() {
        // Missing private_key
    ...
    async fn test_send_push_with_custom_data() {
        ...
        let result = send_push(
            &client,
            "my-project",
            &REDACTED_SECRET_json("http://127.0.0.1:0"),
            "token",
            "Title",
            "Body",
            Some(&data),
    ...
    async fn test_get_access_token_endpoint_returns_error() {
        ...
        let client = reqwest::Client::new();
        let sa_json = REDACTED_SECRET_json(&format!("{}/token", server.uri()));
    ...
    async fn test_send_push_full_success() {
        ...
        let client = reqwest::Client::new();
        let sa_json = REDACTED_SECRET_json(&format!("{}/token", token_server.uri()));
        let result = send_push_internal(PushRequest {
            http_client: &client,
            project_id: "test-project",
            REDACTED_SECRET_json: &sa_json,
            token: "device_token",
            title: "Test Title",
            body: "Test Body",
            data: None,
